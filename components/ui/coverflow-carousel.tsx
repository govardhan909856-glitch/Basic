...
export interface CoverflowCarouselProps {
  ...
  onCardClick?: (index: number, slide: CoverflowSlide) => void;
...
export function CoverflowCarousel({
  ...
  const settle = React.useCallback(
    ...
    [indexAt, paint, onActiveIndexChange],
  );
          ...
          >
            {slides.map((slide, index) => (
              <div
                ...
                onClick={(e) => {
                  // If clicked near centre, select/trigger card click
                  if (index === selected) {
                    if (slide.onClick) slide.onClick();
                    if (onCardClick) onCardClick(index, slide);
                  } else {
                    goTo(index);
                  }
                }}
      ...
      {showCaption && active?.title && (
        ...
        >
          ...
          {active.meta && active.meta.length > 0 && (
            <dl className="mt-4 w-full max-w-md text-xs font-mono grid grid-cols-1 sm:grid-cols-3 gap-2 bg-white/[0.03] border border-white/[0.08] p-3 rounded-xl">
              {active.meta.map((row) => (
                <div key={row.label} className="flex flex-col items-center text-center p-1">
